...
<#assign themeDisplay = objectUtil("com.liferay.portal.kernel.theme.ThemeDisplay") />
...
<hr>
  ...
  <div class="row" style="padding-bottom: 50px">
		<#list rowList as member>
			...
			<#assign profileUrl = "http://localhost:8080/web/${htmlUtil.escape(member.getScreenName())}/profile" />
			<#assign profilePic = member.getPortraitURL(themeDisplay) />

	    <div style="text-align: center" class="col-md-${cellSize} hoverdark img-rounded content-animation">
		    <div class="circle-image" style="background-image: url(${profilePic});"></div>
		    <br>
		    <a target="_blank" href="http://localhost:8080/web/${htmlUtil.escape(member.getScreenName())}/profile"><strong>${htmlUtil.escape(member.getFullName())}</strong></a>
		    <br>
