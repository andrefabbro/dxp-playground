<%@ page import="com.liferay.websocket.example.echo.constants.EchoPortletKeys" %>
<%@ include file="/init.jsp" %>

<h2>WebSockets demo using Liferay 7</h2>
<button onclick="clearMap();">Clear Map</button>
<%--<button onclick="switchSource();">Java/Node Switch</button>--%>
<div id="map_canvas" style="width: 800px; height: 550px;">map</div>
...
<script type="text/javascript">
	...
	var myWorld = {
	        ...
	        "San Luis PotosÃ­,Mexico": {"lat": 38.6270025, "lng": -90.19940419999999},
    ...
    function MapObj(linkItems) {
        this.current = undefined;
        this.size = 0;
        if(linkItems === false)
            this.disableLinking();
    }
    MapObj.noop = function() {
        return this;
    };
    MapObj.illegal = function() {
    ...
    MapObj.from = function(obj, foreignKeys) {
        var map = new MapObj;
        for(var prop in obj) {
    ...
    MapObj.prototype.disableLinking = function() {
        this.link = MapObj.noop;
        this.unlink = MapObj.noop;
        this.disableLinking = MapObj.noop;
        this.next = MapObj.illegal;
        this.key = MapObj.illegal;
        this.value = MapObj.illegal;
        this.removeAll = MapObj.illegal;
        return this;
    };
    // overwrite in MapObj instance if necessary
    MapObj.prototype.hash = function(value) {
        return (typeof value) + ' ' + (value instanceof Object ?
                        (value.__hash || (value.__hash = ++arguments.callee.current)) :
                        value.toString());
    };
    MapObj.prototype.hash.current = 0;
    // --- mapping functions
    MapObj.prototype.get = function(key) {
        var item = this[this.hash(key)];
        return item === undefined ? undefined : item.value;
    };
    MapObj.prototype.put = function(key, value) {
    ...
    MapObj.prototype.remove = function(key) {
        var hash = this.hash(key);
        var item = this[hash];
        if(item !== undefined) {
            --this.size;
    ...
    MapObj.prototype.removeAll = function() {
        while(this.size)
            this.remove(this.key());
        return this;
    };
    // --- linked list helper functions
    MapObj.prototype.link = function(item) {
        if(this.size == 0) {
            item.prev = item;
            item.next = item;
            this.current = item;
    ...
    MapObj.prototype.unlink = function(item) {
        if(this.size == 0)
            this.current = undefined;
        else {
    ...
    MapObj.prototype.next = function() {
        this.current = this.current.next;
    };
    MapObj.prototype.key = function() {
        return this.current.key;
    };
    MapObj.prototype.value = function() {
        return this.current.value;
    };

    var liferay_ws = new WebSocket('<%= "ws://" + request.getServerName() + ":" + request.getServerPort() + EchoPortletKeys.ECHO_WEBSOCKET_PATH %>');
    var addressCache = MapObj.from(myWorld);
    var pointArray;
...
//    }
    ...
    function initialize() {
        ...
        liferay_ws.onopen = function(evt) {
			console.log('CONNECTED TO WEBSOCKET');
		};
		liferay_ws.onclose = function(evt) {
		    console.log('DISCONNECTED FROM WEBSOCKET');
		};
		liferay_ws.onerror = function(evt) {
...
<script type="text/javascript" src="http://maps.google.com/maps/api/js?key=AIzaSyB1yVfjcoahpMf-LMXrRqJxjaTYrOBX08M&callback=initialize&sensor=false&libraries=visualization"></script>
