...
import com.liferay.socialdriver.constants.SocialDriverPortletKeys;
...
@Component(
	...
	property = {
		...
		"javax.portlet.display-name=Social Driver Portlet",
		"javax.portlet.init-param.template-path=/",
		"javax.portlet.init-param.view-template=/view.jsp",
		"javax.portlet.name=" + SocialDriverPortletKeys.SocialDriver,
...
public class SocialDriverPortlet extends MVCPortlet {
	...
	private final SocialDriverContentUtil contentContainer = new SocialDriverContentUtil();
	...
	public void serveResource(ResourceRequest request, ResourceResponse response) throws IOException {
		...
		try {
			...
			int count = MBMessageLocalServiceUtil.getGroupMessagesCount(groupId, userId,
		...
		try {
			...
			if (!GetterUtil.getString(request.getParameter("startBlog")).isEmpty()) {
				if (blogThread != null) {
					if (blogThread.getState() == Thread.State.TERMINATED) {
						blogThread = new BlogPoster(companyId, groupId, themeId, profileFlag, contentContainer);
						blogThread.turnOn();
						blogThread.start();
					} else {
						blogThread.turnOn();
			...
			if (!GetterUtil.getString(request.getParameter("stopBlog")).isEmpty()) {
				blogThread.turnOff();
			}
			if (!GetterUtil.getString(request.getParameter("clearBlog")).isEmpty()) {
				SocialDriverUtil.clearBlogs(groupId);
			}
			if (!GetterUtil.getString(request.getParameter("startWiki")).isEmpty()) {
				if (wikiThread != null) {
					if (wikiThread.getState() == Thread.State.TERMINATED) {
						wikiThread = new WikiPoster(companyId, groupId, themeId, profileFlag, contentContainer);
						wikiThread.turnOn();
						wikiThread.start();
					} else {
						wikiThread.turnOn();
			...
			if (!GetterUtil.getString(request.getParameter("clearMB")).isEmpty()) {
				SocialDriverUtil.clearMB(companyId);
			}
			if (!GetterUtil.getString(request.getParameter("clearUsers")).isEmpty()) {
				SocialDriverUtil.clearUsers(companyId, user.getUserId());
			}
			if (!GetterUtil.getString(request.getParameter("startMB")).isEmpty()) {
				if (mbThread != null) {
					if (mbThread.getState() == Thread.State.TERMINATED) {
						mbThread = new MBPoster(companyId, groupId, themeId, profileFlag, contentContainer);
						mbThread.turnOn();
						mbThread.start();
					} else {
						mbThread.turnOn();
			...
			if (!GetterUtil.getString(request.getParameter("stopWiki")).isEmpty()) {
				wikiThread.turnOff();
			}
			if (!GetterUtil.getString(request.getParameter("clearBlog")).isEmpty()) {
				SocialDriverUtil.clearWikis(groupId);
			}
			if (!GetterUtil.getString(request.getParameter("genStats")).isEmpty()) {
	...
	private void generateSocialActivity(long groupId) throws Exception {
		List<SocialActivityCounter> counters = SocialActivityCounterLocalServiceUtil
				.getSocialActivityCounters(QueryUtil.ALL_POS, QueryUtil.ALL_POS);
		...
		for (User user : UserLocalServiceUtil.getUsers(QueryUtil.ALL_POS, QueryUtil.ALL_POS)) {
			...
			for (int off = -12; off <= 0; off++) {
				...
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, userClassId, user.getUserId(),
						"user.blogs", SocialActivityCounterConstants.TYPE_ACTOR, pub + nub, 0,
				...
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, userClassId, user.getUserId(),
						"user.wikis", SocialActivityCounterConstants.TYPE_ACTOR, puw + nuw, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				c.setStartPeriod(startPeriod);
				c.setEndPeriod(endPeriod);
				...
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, userClassId, user.getUserId(),
						"user.blog-updates", SocialActivityCounterConstants.TYPE_ACTOR, nubu + pubu, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				c.setStartPeriod(startPeriod);
				c.setEndPeriod(endPeriod);
				...
				SocialActivityCounterLocalServiceUtil.updateSocialActivityCounter(c);
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, userClassId, user.getUserId(),
						"user.wiki-updates", SocialActivityCounterConstants.TYPE_ACTOR, puwu + nuwu, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				c.setStartPeriod(startPeriod);
				c.setEndPeriod(endPeriod);
				...
				SocialActivityCounterLocalServiceUtil.updateSocialActivityCounter(c);
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, userClassId, user.getUserId(),
						"user.message-posts", SocialActivityCounterConstants.TYPE_ACTOR, pump + nump, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				c.setStartPeriod(startPeriod);
				c.setEndPeriod(endPeriod);
				...
				SocialActivityCounterLocalServiceUtil.updateSocialActivityCounter(c);
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, userClassId, user.getUserId(),
						"user.activities", SocialActivityCounterConstants.TYPE_ACTOR, pua + nua, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				...
				SocialActivityCounterLocalServiceUtil.updateSocialActivityCounter(c);
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, userClassId, user.getUserId(),
						"user.comments", SocialActivityCounterConstants.TYPE_ACTOR, puc + nuc, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				...
				SocialActivityCounterLocalServiceUtil.updateSocialActivityCounter(c);
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, userClassId, user.getUserId(),
						"user.votes", SocialActivityCounterConstants.TYPE_ACTOR, puv + nuv, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				...
				SocialActivityCounterLocalServiceUtil.updateSocialActivityCounter(c);
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, userClassId, user.getUserId(),
						"participation", SocialActivityCounterConstants.TYPE_ACTOR, pp + np, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				...
				SocialActivityCounterLocalServiceUtil.updateSocialActivityCounter(c);
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, userClassId, user.getUserId(),
						"contribution", SocialActivityCounterConstants.TYPE_CREATOR, pc + nc, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				c.setStartPeriod(startPeriod);
				c.setEndPeriod(endPeriod);
				...
				SocialActivityCounterLocalServiceUtil.updateSocialActivityCounter(c);
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, userClassId, user.getUserId(),
						"creator.subscriptions", SocialActivityCounterConstants.TYPE_CREATOR, pcs + ncs, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				c.setStartPeriod(startPeriod);
				c.setEndPeriod(endPeriod);
				...
				SocialActivityCounterLocalServiceUtil.updateSocialActivityCounter(c);
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, userClassId, user.getUserId(),
						"creator.comments", SocialActivityCounterConstants.TYPE_CREATOR, pcc + ncc, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				c.setStartPeriod(startPeriod);
				c.setEndPeriod(endPeriod);
		...
		for (BlogsEntry blog : BlogsEntryLocalServiceUtil.getBlogsEntries(QueryUtil.ALL_POS, QueryUtil.ALL_POS)) {
			...
			for (int off = -12; off <= 0; off++) {
				...
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, blogClassId, blog.getEntryId(),
						"asset.activities", SocialActivityCounterConstants.TYPE_ASSET, naa + paa, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				...
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, blogClassId, blog.getEntryId(),
						"asset.comments", SocialActivityCounterConstants.TYPE_ASSET, nac + pac, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				c.setStartPeriod(startPeriod);
				c.setEndPeriod(endPeriod);
				...
				SocialActivityCounterLocalServiceUtil.updateSocialActivityCounter(c);
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, blogClassId, blog.getEntryId(),
						"asset.replies", SocialActivityCounterConstants.TYPE_ASSET, nar + par, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				c.setStartPeriod(startPeriod);
				c.setEndPeriod(endPeriod);
				...
				SocialActivityCounterLocalServiceUtil.updateSocialActivityCounter(c);
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, blogClassId, blog.getEntryId(),
						"asset.subscriptions", SocialActivityCounterConstants.TYPE_ASSET, pas + nas, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				c.setStartPeriod(startPeriod);
				c.setEndPeriod(endPeriod);
				...
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, blogClassId, blog.getEntryId(),
						"popularity", SocialActivityCounterConstants.TYPE_ASSET, pp + np, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				c.setStartPeriod(startPeriod);
				c.setEndPeriod(endPeriod);
				c.setTotalValue(pp + np);
				...
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, blogClassId, blog.getEntryId(),
						"asset.votes", SocialActivityCounterConstants.TYPE_ASSET, pav + nav, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				c.setStartPeriod(startPeriod);
				c.setEndPeriod(endPeriod);
		...
		for (WikiPage wiki : WikiPageLocalServiceUtil.getWikiPages(QueryUtil.ALL_POS, QueryUtil.ALL_POS)) {
			...
			for (int off = -12; off <= 0; off++) {
				...
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, wikiClassId, wiki.getPrimaryKey(),
						"asset.activities", SocialActivityCounterConstants.TYPE_ASSET, naa + paa, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				c.setStartPeriod(startPeriod);
				c.setEndPeriod(endPeriod);
				...
				SocialActivityCounterLocalServiceUtil.updateSocialActivityCounter(c);
				c = SocialActivityCounterLocalServiceUtil.addActivityCounter(groupId, wikiClassId, wiki.getPrimaryKey(),
						"asset.comments", SocialActivityCounterConstants.TYPE_ASSET, pac + nac, 0,
						SocialActivityCounterConstants.PERIOD_LENGTH_SYSTEM);
				c.setStartPeriod(startPeriod);
				c.setEndPeriod(endPeriod);
