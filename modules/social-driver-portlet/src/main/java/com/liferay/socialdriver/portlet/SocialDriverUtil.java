...
public class SocialDriverUtil {
	...
	public static long getUserId(long companyId, String themeId, boolean profileFlag) throws Exception {
	...
	public static void configureJournalContent(Layout layout, Group group, String portletId, String articleId)
		...
		PortletPreferences portletSetup = (PortletPreferences) PortletPreferencesFactoryUtil.getLayoutPortletSetup(layout, portletId);
	...
	public synchronized static User addUser(String firstName, long companyId, String themeId, boolean profileFlag)
			throws Exception {
		...
		try {
			...
			// group if so
			Group salesGroup = GroupLocalServiceUtil.getGroup(companyId, "Sales");
			groupIds = new long[] { guestGroup.getGroupId(), salesGroup.getGroupId() };
		} catch (Exception ex) {
		...
		try {
			...
			// user if so
			roleIds = new long[] { RoleLocalServiceUtil.getRole(companyId, "Social Office User").getRoleId() };
			serviceContext.setScopeGroupId(guestGroup.getGroupId());
		} catch (Exception ignored) {
			roleIds = new long[] {};
		}
		User user = UserLocalServiceUtil.addUser(UserLocalServiceUtil.getDefaultUserId(companyId), companyId, false,
				firstName, firstName, false, firstName.toLowerCase(), firstName.toLowerCase() + "@liferay.com", 0, "",
				LocaleUtil.getDefault(), firstName, "", lastName, 0, 0, true, 8, 15, 1974, job, groupIds, new long[] {},
				roleIds, new long[] {}, false, serviceContext);
		...
		/**
		 * if (profileFlag) { setupUserProfile(companyId, themeId, guestGroup,
		 * user); }
		 **/
		return user;
	 ...
	 * Group guestGroup, User user) throws Exception { Group group =
	 * user.getGroup(); if (themeId != null && !themeId.isEmpty()) {
	 * LayoutSetLocalServiceUtil.updateLookAndFeel( group.getGroupId(), false,
	 * themeId, "01", "", false); }
	 * 
	 * // Profile layout
	 * 
	 * Layout layout = addLayout( group, user.getFullName(), false, "/profile",
	 * "2_columns_ii"); JournalArticle cloudArticle, assetListArticle;
	 * 
	 * try { cloudArticle = JournalArticleLocalServiceUtil.getLatestArticle
	 * (guestGroup.getGroupId(), SocialDriverConstants.CLOUD_ARTICLE_ID); }
	 * catch (NoSuchArticleException ex) { cloudArticle =
	 * addArticle("/cloud-structure.xml", "/cloud-template.vm",
	 * "cloud-article.xml", UserLocalServiceUtil.getDefaultUserId(companyId),
	 * guestGroup.getGroupId(), SocialDriverConstants.CLOUD_ARTICLE_ID); }
	 * 
	 * try { assetListArticle = JournalArticleLocalServiceUtil
	 * .getLatestArticle(guestGroup.getGroupId(),
	 * SocialDriverConstants.ASSETLIST_ARTICLE_ID); } catch
	 * (NoSuchArticleException ex) { assetListArticle =
	 * addArticle("/assetlist-structure.xml", "/assetlist-template.vm",
	 * "assetlist-article.xml",
	 * UserLocalServiceUtil.getDefaultUserId(companyId),
	 * guestGroup.getGroupId(), SocialDriverConstants.ASSETLIST_ARTICLE_ID); }
	 * try { JournalArticleLocalServiceUtil.getLatestArticle(guestGroup
	 * .getGroupId(), SocialDriverConstants.EXPERTSLIST_ARTICLE_ID); } catch
	 * (NoSuchArticleException ex) { addArticle("/experts-structure.xml",
	 * "/experts-template.vm", "experts-article.xml",
	 * UserLocalServiceUtil.getDefaultUserId(companyId),
	 * guestGroup.getGroupId(), SocialDriverConstants.EXPERTSLIST_ARTICLE_ID); }
	 * 
	 * addPortletId(layout, "1_WAR_socialnetworkingportlet", "column-1");
	 * addPortletId(layout, PortletKeys.REQUESTS, "column-1"); String portletId
	 * = addPortletId(layout, PortletKeys.JOURNAL_CONTENT, "column-1");
	 * configureJournalContent( layout, guestGroup, portletId,
	 * cloudArticle.getArticleId()); configurePortletTitle(layout, portletId,
	 * "Expertise"); addPortletBorder(layout, portletId);
	 * 
	 ...
	 * "column-2")); addPortletBorder(layout, addPortletId(layout,
	 ...
	 * 
	 * addPortletId(layout, "1_WAR_socialnetworkingportlet", "column-1");
	 * addPortletId(layout, PortletKeys.REQUESTS, "column-1"); portletId =
	 * addPortletId(layout, PortletKeys.JOURNAL_CONTENT, "column-1");
	 * configureJournalContent( layout, guestGroup, portletId,
	 * cloudArticle.getArticleId()); configurePortletTitle(layout, portletId,
	 * "Expertise"); addPortletBorder(layout, portletId);
	 ...
	 * "column-2"); configureJournalContent( layout, guestGroup, portletId,
	 * assetListArticle.getArticleId()); configurePortletTitle(layout,
	 * portletId, user.getFirstName() + "'s " + "Contributions");
	 ...
	 * addPortletBorder(layout, addPortletId(layout,
	 * "2_WAR_socialnetworkingportlet", "column-1")); addPortletBorder(layout,
	 ...
	 * 
	 * addPortletBorder(layout, addPortletId(layout,
	 * PortletKeys.RECENT_BLOGGERS, "column-1")); addPortletBorder(layout,
	 * addPortletId(layout, PortletKeys.BLOGS, "column-2"));
	 * 
	 * // Workspace layout
	 * 
	 * layout = addLayout( group, "Workspace", false, "/workspace",
	 * "2_columns_i");
	 * 
	 * addPortletBorder(layout, addPortletId(layout,
	 * PortletKeys.RECENT_DOCUMENTS, "column-1")); addPortletBorder(layout,
	 * addPortletId(layout, PortletKeys.DOCUMENT_LIBRARY, "column-2"));
	 * 
	...
	private static void assignAddressTo(User user) throws SystemException, PortalException {
		...
		AddressLocalServiceUtil.addAddress(user.getUserId(), Contact.class.getName(), user.getContactId(), street,
				StringPool.BLANK, StringPool.BLANK, city, zip, regionId, country.getCountryId(), 11002, true, true,
				new ServiceContext());
	}
	...
	/**
	 * public static JournalArticle addJournalArticle( long userId, long
	 * groupId, String title, String fileName, String structureId, String
	 * templateId, ServiceContext serviceContext) throws Exception {
	 * 
	 ...
	 * 
	 * setLocalizedValue(titleMap, title); try {
	 * JournalArticleLocalServiceUtil.deleteArticle(groupId, title,
	 * serviceContext); } catch (Exception ex) { System.out.println("Ignoring "
	 * + ex.getMessage()); }
	 * 
	 * //return null;
	 * 
	 * return JournalArticleLocalServiceUtil.addArticle( userId, groupId, 0, 0,
	 * 0, title, false, JournalArticleConstants.VERSION_DEFAULT, titleMap, null,
	 * content, "general", structureId, templateId, StringPool.BLANK, 1, 1,
	 * 2008, 0, 0, 0, 0, 0, 0, 0, true, 0, 0, 0, 0, 0, true, true, false,
	 * StringPool.BLANK, null, null, StringPool.BLANK, serviceContext); }
	 * 
	 * public static JournalStructure addJournalStructure( long userId, long
	 * groupId, String title, String fileName) throws Exception {
	 * 
	 ...
	 * serviceContext.setAddGuestPermissions(true);
	 * 
	 * try { JournalStructureLocalServiceUtil.deleteStructure(groupId, title); }
	 * catch (Exception ex) { System.out.println("Ignoring " + ex.getMessage());
	 * } return JournalStructureLocalServiceUtil.addStructure( userId, groupId,
	 * title, false, StringPool.BLANK, nameMap, descriptionMap, xsd,
	 * serviceContext); }
	 * 
	 * public static JournalTemplate addJournalTemplate( long userId, long
	 * groupId, String title, String structureId, String fileName) throws
	 * Exception {
	 * 
	 * Map<Locale, String> descriptionMap = new HashMap<Locale, String>();
	 * 
	 ...
	 * serviceContext.setAddGuestPermissions(true);
	 * 
	 * try { JournalTemplateLocalServiceUtil.deleteTemplate(groupId, title); }
	 * catch (Exception ex) { System.out.println("Ignoring " + ex.getMessage());
	 * 
	 * } return JournalTemplateLocalServiceUtil.addTemplate( userId, groupId,
	 * title, false, structureId, nameMap, descriptionMap, xsl, true, "vm",
	 * false, false, StringPool.BLANK, null, serviceContext); }
	 * 
	 * public static JournalArticle addArticle(String structPath, String
	 * tmplPath, String articlePath, long userId, long groupId, String
	 * articleId) throws Exception { JournalStructure struct =
	 * addJournalStructure(userId, groupId, articleId, structPath);
	 * JournalTemplate tmpl = addJournalTemplate(userId, groupId, articleId,
	 * struct.getStructureId(), tmplPath); ServiceContext serviceContext = new
	 * ServiceContext(); serviceContext.setScopeGroupId(groupId);
	 * serviceContext.setAddGroupPermissions(true);
	 * serviceContext.setAddGuestPermissions(true); return addJournalArticle(
	 * userId, groupId, articleId, articlePath, struct.getStructureId(),
	...
	// layoutset
	/**
	 * public static Layout addLayout( Group group, String name, boolean
	 * privateLayout, String friendlyURL, String layoutTemplateId) throws
	 * Exception {
	 * 
	 * ServiceContext serviceContext = new ServiceContext();
	 * 
	...
	public static void clearUsers(long companyId, long curUser) throws Exception {
