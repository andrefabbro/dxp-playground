...
public class MBPoster extends Thread {
	...
	public MBPoster(long companyId, long groupId, String themeId, boolean profileFlag,
	...
	private void replyMsg() throws Exception {
		...
		if (MBMessageLocalServiceUtil.getGroupMessagesCount(groupId, WorkflowConstants.STATUS_APPROVED) <= 0) {
			return;
		}

		int rand = (int) (Math.random()
				* (double) MBMessageLocalServiceUtil.getGroupMessagesCount(groupId, WorkflowConstants.STATUS_APPROVED));
		MBMessage msg = MBMessageLocalServiceUtil
				.getGroupMessages(groupId, WorkflowConstants.STATUS_APPROVED, rand, rand + 1).get(0);
		long categoryId = msg.getCategoryId();
		long threadId = msg.getThreadId();
		long parentId = msg.getMessageId();
		MBMessageLocalServiceUtil.addMessage(SocialDriverUtil.getUserId(companyId, themeId, profileFlag), "Joe Schmoe",
				groupId, categoryId, threadId, parentId, "RE: " + msg.getSubject(), content, "html",
				new ArrayList<ObjectValuePair<String, InputStream>>(), false, 1.0, true, context);

		System.out.println("replied to msg " + msg.getSubject() + " in " + "category " + msg.getCategory().getName());
	}
	...
	private void addMsg() throws Exception {
		...
		MBCategory newcat = MBCategoryLocalServiceUtil.addCategory(userId, 0, StringUtil.shorten(title, 70),
				"Discussions about " + title, MBCategoryConstants.DEFAULT_DISPLAY_STYLE, null, null, null, 0, false,
				null, null, -1, null, false, null, 0, false, null, null, false, false, context);

		MBMessageLocalServiceUtil.addMessage(SocialDriverUtil.getUserId(companyId, themeId, profileFlag), "Hoe Schmoe",
				groupId, newcat.getCategoryId(), 0, 0, title, content, "html",
				new ArrayList<ObjectValuePair<String, InputStream>>(), false, 1.0, true, context);
		
		System.out.println("added msg " + title + " in category " + newcat.getName());
